...
public class AutoParkAgent : Agent
{
    ...
    private void OnCollisionEnter(Collision other)
    {
            ...
            other.gameObject.CompareTag("tree"))
        {
            //logging the failure
            Debug.Log("collided");
            this.gameObject.GetComponent<EfficiencyCal>().collisions++;
            this.gameObject.GetComponent<EfficiencyCal>().CalEfficiency();
    ...
    public void JackpotReward(float bonus)
    {
        if(bonus > 0.2f)
            Debug.LogWarning("Jackpot hit! " + bonus);
        AddReward(0.2f + bonus);
        
        //when this fxn was a coroutine, it waited for the End of Frame to execute EndEpisode(), agent took advantage of it and always kept on moving to avoid end of frame while colliding with the parking collider and stealing reward points.
        //yield return new WaitForEndOfFrame();
        
        /*_rigitBody.velocity = Vector3.zero; //Braking the car (Failed Attempt) */
